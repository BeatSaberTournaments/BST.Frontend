...
import { Tournament, MapPool } from '../interfaces/tournaments';
...
export async function getTournamentMapools(id: number) {
    const result = await Information.query(`SELECT id,mappools,public FROM tournaments WHERE id = ${id}`);
...
export async function getTournamentMapool(id: number, mid:number) {
    ...
    if (result.rows.length === 0) {
        ...
        const mappool = JSON.parse(result.rows[0].mappools).find(x => x.id == mid);
...
export async function getAllTournaments() {
    const result = await Information.query('SELECT id,tournamentname,state,startdate,enddate,signupstatus,public,image FROM tournaments ORDER by ID ASC');
    if (result.rows.length === 0) {
        return "Not found";
    } else if (!result.rows[0].public) {
        return "Not public";
...
export async function getTournamentsCalendar() {
    const result = await Information.query(`SELECT id,tournamentname,state,startdate,enddate,signupstatus,public,image FROM tournaments WHERE enddate > NOW() AND state != 3 ORDER by startdate ASC`);
    if (result.rows.length === 0) {
        return "Not found";
    } else if (!result.rows[0].public) {
        return "Not public";
